 ...
 */
export function normalizeWebSessionVolatiles(log: string, workspaceCwds: readonly string[] = []): string {
  ...
  const cwdSpellings = [...new Set([sessionCwd, ...workspaceCwds]
  ...
  return log.split(/\r?\n/).map((line) => {
    ...
    const record = mapJsonStringValues(JSON.parse(line), (value, key) => {
      if (key === 'clientTimeZone') return 'UTC'
...
async function assertReplaySession(
  ...
  const actualSnapshot = normalizeSessionSnapshots([normalizeWebSessionVolatiles(actual, [workspaceCwd])], actualContext)[0]
    ?.split(harnessHome).join('{{harnessHome}}')
