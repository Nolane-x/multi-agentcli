...
describe('Issue lifecycle workflow', () => {
  ...
  it('keeps fork policy validation read-only without an upstream Project token', () => {
    const policy = loadWorkflow('.github/workflows/issue-policy.yml')
    ...
    expect(steps.find(step => step.name === 'Create Project read token')).toBeUndefined()
    ...
    expect(validateStep?.run).toContain('configured_repository')
    expect(validateStep?.run).toContain('Passing without applying upstream policy')
  })
})
