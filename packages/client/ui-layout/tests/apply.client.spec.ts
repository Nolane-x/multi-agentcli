...
describe('ui-layout client apply', () => {
  it('declares its service dependencies', () => {
    expect(inject).toEqual(['slots', 'theme', 'locale', 'sessions', 'uiRenderer'])
  })
  ...
  it('injects spatial session actions and attaches the layout actions', async () => {
    const { ctx, slots } = await bench()
    const fiber = ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    const actions = {
    ...
    expect(Object.keys(injected).sort()).toEqual(['SessionScope', 'openAgent', 'stageAgent', 'stopAgentJob'])
