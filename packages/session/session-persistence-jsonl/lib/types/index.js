...
import { SessionPersistence, SessionPersistenceRevision, SessionFormatUnsupportedError, SessionPersistenceCorruptionError, SessionAlreadyExistsError, SessionPersistenceNotFoundError, assertStoredId, assertVersion, materializeCreateHeader, validateStoredEvents, } from '@deepseek-ai/dsh-session-persistence';
import { JsonlBackendTracker, JsonlSessionHandle } from "./storage.js";
import { SessionLogOffset } from '@deepseek-ai/dsh-session';
import { encodeSegment, eventLines, logPath, logSuffix, parseHeader, parseHeaderMeta, projectDir, scanLog, sessionDir, SessionLogScanner, toHeaderLine, } from "./format.js";
...
class JsonlSessionPersistence extends SessionPersistence {
    ...
    async stat(id, options) {
        ...
        options?.signal?.throwIfAborted();
        if (first === undefined)
            return undefined; // empty/half-written file
        let stored;
        try {
    ...
    async readStableFile(path, signal) {
        ...
        for (let attempt = 0;; attempt += 1) {
            ...
            if (before === fileRevision(after))
                return { buffer, revision: before };
            if (attempt === 1) {
