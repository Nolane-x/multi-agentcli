...
let MessageFeedbackService = (() => {
    ...
    return class MessageFeedbackService extends _classSuper {
        ...
        async observeSession(sessionId) {
            ...
            if (live !== undefined)
                return { meta: live.header, events: live.snapshotEvents() };
        ...
        async ensureTargetDurable(observation) {
            ...
            if (live !== undefined && sameHeaderIdentity(live.header, observation.meta)) {
                if (!(await this.ctx.sessions.flush(live))) {
                    throw new Error(`message-feedback: no durability listener participated for live session '${observation.meta.id}'`);
                }
            }
