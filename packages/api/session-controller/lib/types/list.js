...
export class ApiSessionList {
    ...
    async probeSmallCold(header, signal) {
        ...
        if (coldBlankProbeMaxEvents === 0 && coldBlankProbeMaxBytes === 0)
            return undefined;
        ...
        if (snapshot.eventCount !== undefined) {
            if (coldBlankProbeMaxEvents === 0 || snapshot.eventCount > coldBlankProbeMaxEvents)
                return undefined;
        }
        else if (snapshot.sizeBytes !== undefined) {
            if (coldBlankProbeMaxBytes === 0 || snapshot.sizeBytes > coldBlankProbeMaxBytes)
                return undefined;
        }
        else {
