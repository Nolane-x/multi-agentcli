...
| `ctx.terminalControlController` | `core` | `session-controller` | - | [`client-ui-layout`](../packages/client/ui-layout) | - | Exposes the exact-Agent PTY control plane over the generated terminal Remote namespace without bypassing Session ownership or the resident terminal registry. |
