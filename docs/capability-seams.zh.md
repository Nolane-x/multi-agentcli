...
flowchart LR
  ...
  svc_terminalControlController["ctx.terminalControlController<br/>Owner-scoped terminal Remote controller"]
  ...
  pkg_session_controller --> svc_terminalControlController
  ...
  svc_terminalControlController --> pkg_client_ui_layout
...
| `ctx.terminalControlController` | `core` | `session-controller` | - | [`client-ui-layout`](../packages/client/ui-layout) | - | 通过生成的 terminal Remote 命名空间暴露精确到 Agent 的 PTY 控制面，不绕过 Session 所有权或常驻终端注册表。 |
